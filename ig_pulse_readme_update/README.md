...
```
                    ┌──────────────────────────┐
                    │   DomainStreamAggregator  │
                    │   (15 streams, no API keys) │
                    └──────────┬───────────────┘
                               │ DomainSignal (primitive alert levels)
                               ▼
                    ┌──────────────────────────┐
                    │   Schema: Snapshot →      │
                    │   snapshots.jsonl         │
              ...
              ▼                ▼                 ▼
      ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
      │   coupler    │ │   grapher    │ │   reporter   │
      │ Pearson r     │ │ ASCII matrix │ │ Propagation  │
      │ cross-lag     │ │ DOT / graph  │ │ anatomy       │
      └──────┬───────┘ └──────┬───────┘ └──────┬───────┘
             │                │                 │
             ▼                ▼                 │
      coupling.json     graph.json              │
      (41 edges)        (nodes + edges)          │
                                                 │
      coupling_edges_network.html ◄──────────────┘
...
```
fear_greed:⊙ → fear_greed:Φ  lag=23177s r=+1.000
fear_greed:Φ → fear_greed:⊙  lag=23177s r=+1.000
```
